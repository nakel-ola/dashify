      ...
      >
        <div className='w-full lg:w-[80%]'>
          <div className='rounded-md border-[1.5px] border-slate-100 dark:border-neutral-800 flex items-center space-x-2 cursor-pointer p-5'>
            <Avatar fallback={`${user?.lastName.charAt(0)}` + user?.firstName.charAt(0)} className='h-[50px] w-[50px]'>
              <AvatarImage src={user?.photoUrl} />
            </Avatar>

            <p className='text-lg font-medium text-black dark:text-white'>{user?.firstName}</p>
          </div>

          <div className='flex mt-5'>
            <Button onClick={() => setIsOpen(!isOpen)} className='w-fit mt-5 ml-auto bg-red-500 hover:bg-red-500/90'>
              Delete my account
            </Button>
          </div>
        </div>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent>
          ...
          <div className='flex items-center gap-5 mt-4'>
            ...
            <Button onClick={handleDelete} className='bg-red-500 hover:bg-red-500/90'>
              Delete
              <MoonLoader size={20} color='white' loading={isLoading} className='ml-2 text-white' />
            </Button>
          </div>
        </DialogContent>
      </Dialog>
