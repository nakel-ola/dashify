  ...
  return (
    <div className="page-max-width flex flex-col items-center pb-20">
      ...
      <div className="w-[90%] lg:w-[80%]">
        <Accordion type="single" collapsible className="w-full pt-10 divide-y divide-slate-100 dark:divide-neutral-700">
          {items.map((item, index) => (
            <AccordionItem key={index} value={`item-${index}`} className="border-0">
              <AccordionTrigger className="text-xl text-start text-black dark:text-white">
                {item.question}
              </AccordionTrigger>
              <AccordionContent className="text-base text-gray-dark dark:text-gray-light">{item.answer}</AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      </div>
